...
public class ErrorMessageUtils {
	...
	public static final String createServereErrorMsgGet(Exception serverException, int getResponseCode){
		...
		LogUtils.warningLog("ErrorMessageUtils", "server_exception = " + serverException + " getResponseCode = " + getResponseCode);
		if(serverException != null && serverException.getMessage().length() > 0){
			...
			if(getResponseCode == 401){
				serverResponseMessage = "Authentication Failed.." ;
				...
				if(getResponseCode == 404){
					...
					serverResponseMessage = "Internal Server Error. Reason can be : Required all fields are not fill up. Please check information";
				}else{
	...
	public static final String createServerErrorMsgPost(Exception serverException, int postResponseCode){
		String serverResponseMessage = null;
		if(serverException != null && serverException.getMessage().length() > 0){
