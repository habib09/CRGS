...
public class InstanceUploaderTask extends AsyncTask<Long, Integer, InstanceUploaderTask.Outcome> {
    ...
    private boolean uploadOneSubmission(String urlString, String id, String instanceFilePath,
            ...
            try {
                ...
                if (statusCode == HttpStatus.SC_UNAUTHORIZED) {
                        ...
                        try {
                            ...
                            if (u.getHost().equalsIgnoreCase(uNew.getHost())) {
                            	...
                            	outcome.mResults.put(
                                    id, fail + "Unexpected redirection attempt to a different host: "
                                            + uNew.toString());
                    ...
                    if (statusCode >= HttpStatus.SC_OK && statusCode < HttpStatus.SC_MULTIPLE_CHOICES) {
                    	outcome.mResults.put(
                            id, fail + "Invalid status code on Head request.  If you have a web proxy, you may need to login to your network. ");
                        cv.put(InstanceColumns.STATUS,
                            InstanceProviderAPI.STATUS_SUBMISSION_FAILED);
                        ClientCollection.getInstance().getContentResolver()
                                .update(toUpdate, cv, null, null);
        ...
        while (j < files.size() || first) {
            ...
            try {
                ...
                Log.i(t, "Response response:" + response );
                ...
                if (responseCode != HttpStatus.SC_CREATED && responseCode != HttpStatus.SC_ACCEPTED) {
                    if (responseCode == HttpStatus.SC_OK) {
                    	...
                    	if(postResponse != null && postResponse.length() > 0){
                               String retVal = processServerResult(postResponse, responseCode);
                    		   //outcome.mResults.put(id, postResponse); //TODO
                               outcome.mResults.put(id, retVal);
                    	}else{
                    		outcome.mResults.put(id, fail + response.getStatusLine().getReasonPhrase()
                                    + " (" + responseCode + ") at " + urlString);
                    	}
    ...
    private String processServerResult(String postResponse, int responseCode){
        String retVal=null;
        try{
            Document doc = DocumentBuilderFactory.newInstance().newDocumentBuilder()
                    .parse(new InputSource(new StringReader(postResponse)));
            if(responseCode != 0){
                retVal = doc.getElementsByTagName("message").item(0).getTextContent();
                Log.d("HIMEL",retVal);
                LogUtils.warningLog(this, "retval = " + retVal);
            }
        }catch (Exception e){
            e.printStackTrace();
            retVal="Success";
        ...
        try {
	        ...
	        if (c.getCount() > 0) {
	            ...
	            while (c.moveToNext()) {
                    ...
                    Log.d(InstanceUploaderTask.class.getSimpleName(), "Update  URI= " + toUpdate);
