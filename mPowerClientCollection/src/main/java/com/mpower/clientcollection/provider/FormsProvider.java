...
public class FormsProvider extends ContentProvider{
	...
	private static class DatabaseHelper extends ClientCollectionSQLiteOpenHelper {
		...
		private void onCreateNamed(SQLiteDatabase db, String tableName) {
			LogUtils.warningLog(this, "onCreateMethod");
	...
	public synchronized Uri insert(Uri uri, ContentValues initialValues) {
		...
		String[] selectionArgs = { filePath, userId };
		String selection = FormsColumns.FORM_FILE_PATH + "=? AND " + FormsColumns.USER_ID + "=?";
		Cursor c = null;
		try {
	...
	private boolean isMultipleUserAssignedwithForm(String jrFormId){
		String selection = FormsColumns.JR_FORM_ID + "=?";
		String[] selectionArgs = {String.valueOf(jrFormId)};
		boolean retVal;
		Cursor cur = ClientCollection.getAppContext().getContentResolver().query(FormsColumns.CONTENT_URI, null, selection, selectionArgs, null);
		if(cur != null && cur.getCount()==1){
			retVal =  true;
		}else{
			LogUtils.warningLog(this,"Should not delete this blank form cause mutiple users are using this form");
			retVal =  false;
		}
		...
		case FORM_ID:
			...
			try {
				...
				if (c.getCount() > 0) {
					c.moveToFirst();
					do {
						LogUtils.informationLog(this, "jr_form_id = " + c.getString(c.getColumnIndex(FormsColumns.JR_FORM_ID)));
						boolean couldDelete = isMultipleUserAssignedwithForm(c.getString(c.getColumnIndex(FormsColumns.JR_FORM_ID)));
						if(couldDelete){
	...
	static {
		...
		sFormsProjectionMap.put(FormsColumns.JR_FORM_ID,
				FormsColumns.JR_FORM_ID);
		sFormsProjectionMap.put(FormsColumns.USER_ID,
				FormsColumns.USER_ID);
