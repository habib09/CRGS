...
public class PushService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (!UserCollection.getInstance().isLoggedin()) {
			...
			LogUtils.warningLog(this, "User is not loged in (OR hint: logged info is not stored in UserCollection class of Clientcollection)");
			stop();
			stopSelf();
		} else {
...
*/		
	...
	private String getHost() {
		...
		//String url = "http://192.168.22.114:8001";
		...
		LogUtils.informationLog(this, "Mosquitto host: " + retval);
		return retval;
	}
	...
	private class MQTTConnection implements MqttCallback {
		...
		class Connector implements Callable<Void> {
			...
			public Void call() {
				...
				String mqttPort =  preference.getString(PreferencesActivity.KEY_MQTT_SERVER_PORT, getString(R.string.mqtt_port));
				...
				try {
					...
					log("Failed to Connect host: " + host + " Port: " + mqttPort);
					Log.e(TAG,
							"MqttException: "
		...
		public void subscribeToTopics() throws MqttException {
			if ((mqttClient != null) && (mqttClient.isConnected())) {
				...
				/*String topic = "/" + getResources().getString(R.string.app_name) + "/" + API_VERSION + "/"
						+ getUserName();*/
				int[] qos = {MQTT_QUALITY_OF_SERVICE};
				String topic = "/CSA/" + API_VERSION + "/"
						+ getUserName();
				String topic2 = "/CSA/" + API_VERSION;
				String[] topicFilters = {topic, topic2};

				LogUtils.warningLog(this, "Subscribed to: " + topic);
		...
		public void messageArrived(MqttTopic topic, MqttMessage message)
			...
			wl_message.acquire();
			LogUtils.warningLog(this, "***********Message Arrive****");
			Log.w(TAG, "  Topic:\t " + topic.getName() + "\n" + "  Message:\t "
			...
			if (TOPIC_NOTIFICATION.equalsIgnoreCase(new String(message
				...
				if(ClientCollection.getInstance().getIsUseLibraryNotification()){
					LogUtils.informationLog(this, "Using library application(client collection) notification methodology...");
					...
					LogUtils.informationLog(this, "Using target application notification methodology...where status = " + ClientCollection.getInstance().getIsUseLibraryNotification());
					ClientCollection.getInstance().syncTargetApplicationNotificationData(new String(message.getPayload()));
				}
			}else{
				if(ClientCollection.getInstance().getIsUseLibraryNotification()){
					LogUtils.informationLog(this, "Using library application(client collection) notification methodology...");
					ArrayList<MessageInfos> notifyMap = new ArrayList<MessageInfos>();
					MessageInfos info = new MessageInfos("iMEL Message", "0", "0", "", new String(message.getPayload()));
					notifyMap.add(info);
					inserDataIntoDatabase(notifyMap);
